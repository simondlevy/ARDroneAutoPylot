...
const vp_api_stage_funcs_t transcoder_stage_io_file_funcs =
{
  (vp_api_stage_handle_msg_t) NULL,
  (vp_api_stage_open_t) transcoder_stage_io_file_stage_open,
  (vp_api_stage_transform_t) transcoder_stage_io_file_stage_transform,
  (vp_api_stage_close_t) transcoder_stage_io_file_stage_close
};
...
transcoder_stage_io_file_stage_transform(transcoder_stage_io_file_config_t *cfg, vp_api_io_data_t *in, vp_api_io_data_t *out)
{
  ...
  if( out->status == VP_API_STATUS_INIT )
  {
    ...
    out->buffers[out->indexBuffer] = (int8_t *)NULL;
    out->status = VP_API_STATUS_PROCESSING;
    cfg->buffers = out->buffers;
  }
	  ...
	  if(!feof(cfg->f))
	  {
 		  if((fread(&out->size, sizeof(int32_t), 1, cfg->f) > 0) && (out->size > 0))
		  {
			  if(out->size > cfg->max_size)
			  {
				  ...
				  out->buffers[out->indexBuffer] = (int8_t *)vp_os_realloc(out->buffers[out->indexBuffer], sizeof(int8_t) * cfg->max_size);
			  }
	
			  if(!fread(out->buffers[out->indexBuffer], sizeof(int8_t), out->size, cfg->f) == out->size)
